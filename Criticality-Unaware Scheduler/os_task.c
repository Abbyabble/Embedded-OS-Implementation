                         ...
                         INT8U  newprio)
{
    ...
    if (OSRunning == OS_TRUE) {
        //OS_Sched();                                         /* Find new highest priority task          *//AddedCodePA2part1 Comment our
    }
    return (OS_ERR_NONE);
}
#endif
                        ...
                        INT16U   task_or_job)  //TryingPA2part2
{
    ...
    fclose(fp);
    if (prio != 63 && Task_or_Job==0) {
        prio = TaskParameter[id - 1].TaskPriority;
    }
    else if (prio != 63 && Task_or_Job == 1) {
        prio = TaskParameter[id + TASK_NUMBER].TaskPriority;
    }
    /////////////////////
