                 ...
                 INT8U     *perr)
{
    ...
    if (pevent->OSEventCnt == 0)//AddedCodePA3
        return;
...
INT8U  OSSemPost (OS_EVENT *pevent)
{
    ...
    if (pevent->OSEventCnt == 1)//AddedCodePA3
        return;
    ...
    if (pevent->OSEventCnt < 65535u) {                /* Make sure semaphore will not overflow         */
        ...
        //AddedCodePA3
        if ((Output_err = fopen_s(&Output_fp, "./Output.txt", "a")) == 0) {
            if ((OSTimeGet() == (OSTCBCur->OSTCBCyclesStart + OSTCBCur->OSTCBR1Unlock - 2)) && OSTCBCur->OSTCBR1Unlock != 0) {
                printf("%2d\tTask%d release R1\n", OSTimeGet() + 1, OSTCBCur->OSTCBId);
                fprintf(Output_fp, "%2d\tTask%d release R1\n", OSTimeGet() + 1, OSTCBCur->OSTCBId);
                OSTCBCur->OSTCBUsingResource --;
            }
            else if ((OSTimeGet() == (OSTCBCur->OSTCBCyclesStart + OSTCBCur->OSTCBR2Unlock - 2)) && OSTCBCur->OSTCBR2Unlock != 0) {
