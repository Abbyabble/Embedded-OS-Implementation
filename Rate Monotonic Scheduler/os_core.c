                ...
                if (OSPrioHighRdy != OSPrioCur) {          /* No Ctx Sw if current task is highest rdy */
                    ...
                    if ((Output_err = fopen_s(&Output_fp, "./Output.txt", "a")) == 0)
                    {
                        if (OSTCBCur->OSTCBPrio == 63) {
                            printf("%2d\ttask(%2d)\ttask(%2d)(%2d)\tOSIntExit ()\n", OSTime, OSTCBCur->OSTCBPrio, OSTCBHighRdy->OSTCBId, OSTCBHighRdy->OSTCBCtxSwCtr);//AddedCode
                            fprintf(Output_fp, "%2d\ttask(%2d)     \ttask(%2d)(%2d)\tOSIntExit ()\n", OSTime, OSTCBCur->OSTCBPrio, OSTCBHighRdy->OSTCBId, OSTCBHighRdy->OSTCBCtxSwCtr);//AddedCode
                        }
                        else {
                            printf("%2d\ttask(%2d)(%2d)\ttask(%2d)(%2d)\tOSIntExit ()\n", OSTime, OSTCBCur->OSTCBId, OSTCBCur->OSTCBCtxSwCtr, OSTCBHighRdy->OSTCBId, OSTCBHighRdy->OSTCBCtxSwCtr);//AddedCode
...
void  OSStart (void)
{
    if (OSRunning == OS_FALSE) {
        ...
        if ((Output_err = fopen_s(&Output_fp, "./Output.txt", "a")) == 0)
        {
            ...
            printf("%2d\t***********\ttask(%2d)(%2d)\tOSStart ()\n", OSTime, OSTCBHighRdy->OSTCBId, OSTCBHighRdy->OSTCBCtxSwCtr);//AddedCode
        ...
        if (OSLockNesting == 0u) {                     /* ... scheduler is not locked                  */
            ...
            if (OSPrioHighRdy != OSPrioCur) {          /* No Ctx Sw if current task is highest rdy     */
                ...
                if ((Output_err = fopen_s(&Output_fp, "./Output.txt", "a")) == 0)
                {
                    ...
                    printf("%2d\ttask(% 2d)(%2d)\ttask(%2d)\tOS_Sched ()\n", OSTime, OSTCBCur->OSTCBId, OSTCBCur->OSTCBCtxSwCtr, OSTCBHighRdy->OSTCBPrio);// AddedCode
